    ...
    <>
      <header className={`flex bg-white dark:bg-black bg-opacity-70 dark:bg-opacity-70 items-center justify-between w-[100vw] py-3 sticky top-0 left-0 right-0 z-[1000]`}>
        ...
        <div className='flex items-center gap-4 pr-3'>
          ...
          <button className={`md:px-3 md:py-2 p-2 md:text-lg text-xs bg-black text-white dark:bg-white dark:text-black rounded shadow`} onClick={() => { setTheme(theme === 'light' ? 'dark' : 'light') }}>{theme === 'light' ? <BsFillMoonFill color='#fff' /> : <BsFillSunFill color='#ff8400' />}</button>
        </div>
      </header>
    </>
  )
